...
// إنشاء جدول لتخزين تعليقات الأعمدة إذا لم يكن موجودًا
try {
    ...
    // تجاهل الخطأ إذا كان الجدول موجودًا
}
...
// وظيفة لجلب تعليقات الأعمدة لجدول معين
...
function createTable($db, $tableName, $columns) {
    ...
    $db->exec($sql);
    // إدراج التعليقات في جدول التعليقات
...
function dropTable($db, $tableName) {
    // تحقق من أن اسم الجدول يحتوي فقط على أحرف وأرقام وشرطات سفلية
   /*  if (!preg_match('/^[A-Za-z0-9_]+$/', $tableName)) {
        throw new Exception('اسم الجدول غير صالح.');
    } */
...
<body>
    <div class="apex-header">
        <span class="app-title"><i class="fa fa-database"></i> إدارة قواعد بيانات SQLite</span>
        <span class="user-info"><i class="fa fa-user-circle"></i> مستخدم</span>
    </div>
    <nav class="apex-nav">
        <ul>
            ...
            <?php endforeach; ?>
            <li><a href="?action=create_table"><i class="fa fa-plus"></i> إنشاء جدول جديد</a></li>
            <li><a href="?action=custom_query"><i class="fa fa-terminal"></i> استعلام مخصص</a></li>
        </ul>
    </nav>
    <div class="apex-container">
            ...
            <form method="post" action="">
                ...
                <div id="columns_container">
                    <div class="column-row">
                        ...
                        <input type="text" name="column_comment[]" placeholder="تعليق/عنوان العمود">
                    </div>
                </div>
        ...
        <?php elseif (!empty($selectedTable)): ?>
            <!-- عرض بيانات الجدول -->
            <div class="apex-tabs">
                <div class="apex-tab active" onclick="openTab(event, 'browse')">استعراض</div>
            </div>
            
            <div id="browse" class="apex-tab-content active">
                <h2>استعراض الجدول: <?php echo $selectedTable; ?></h2>
                <form method="post" action="" onsubmit="return confirm('هل أنت متأكد من حذف هذا الجدول؟');" style="display:inline-block; margin-bottom:15px;">
                    <input type="hidden" name="action" value="drop_table">
                    <input type="hidden" name="table_name" value="<?php echo $selectedTable; ?>">
                    <input type="submit" value="حذف الجدول" class="apex-btn" style="background:#e55039;">
                </form>
                ...
                <div id="insert_form_container" style="display: none;">
                    <h3 style="color:#3a539b; margin-top:0;">إدراج بيانات في الجدول: <?php echo $selectedTable; ?></h3>
                    <form method="post" action="">
                        ...
                        <?php foreach ($tableColumns as $column): ?>
                            ...
                            <?php if ($column['pk'] && $type === 'INTEGER'): ?>
                                <div style="display:none;">
                                    ...
                                    <input type="<?php echo $inputType; ?>" id="insert_<?php echo $column['name']; ?>" name="data[<?php echo $column['name']; ?>]" placeholder="سيتم توليده تلقائيًا" class="edit-field">
                                </div>
                            <?php else: ?>
                        ...
                        <div style="margin-top: 10px; text-align:left;">
                            <input type="submit" value="إدراج" class="apex-btn">
                            <button type="button" onclick="hideInsertForm()" class="apex-btn" style="background:#e55039;">إلغاء</button>
                        </div>
                    </form>
                </div>
                <script>
                    ...
                    // تأكد أن النموذج مخفي دائمًا عند تحميل الصفحة
